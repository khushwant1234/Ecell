...
export default function Home() {
  ...
  const [playerMoney, setPlayerMoney] = useState(100); // Initial money
  ...
  useEffect(() => {
    ...
    const dealerTurn = async () => {
      ...
      if (dealerValue > 21) {
        ...
        setPlayerMoney((prev) => prev + 10); // Player wins money
      } else if (dealerValue > playerValue) {
        setGameState("dealerWin");
        setPlayerMoney((prev) => prev - 10); // Player loses money
      } else if (dealerValue < playerValue) {
  ...
  const getResultMessage = () => {
    switch (gameState) {
      ...
      case "dealerBust":
        return "Mr. Card Master busts! You win!";
      case "playerWin":
        return "You win!";
      case "dealerWin":
        return "Mr. Card Master wins!";
      case "push":
      ...
      <div
        className={`text-${
          card.suit === "hearts" || card.suit === "diamonds"
            ? "red-600"
            : "black"
        } font-bold text-xl absolute top-2 left-2`}
      >
        {card.value}
      </div>
      ...
      <div
        className={`text-${
          card.suit === "hearts" || card.suit === "diamonds"
            ? "red-600"
            : "black"
        } font-bold text-xl absolute bottom-2 right-2 rotate-180`}
      >
        {card.value}
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-emerald-800 flex flex-col items-center p-4">
      ...
      {/* Money status always visible */}
      <div className="bg-yellow-600 text-white font-bold px-4 py-2 rounded-full mb-4">
      ...
      {gameState === "idle" ? (
        <div className="text-center">
          <p className="text-white text-lg mb-6">
            A classic card game where you try to get as close to 21 as possible
            without going over!
          </p>
          <button
            onClick={startGame}
        ...
        <>
          ...
          {gameState === "playing" && !dealersTurn ? (
            ...
            <div className="text-white text-xl">
              Mr. Card Master is playing...
            </div>
          )}
        </>
      )}

      <div className="mt-auto text-white text-sm opacity-70"></div>
    </div>
  );
}
